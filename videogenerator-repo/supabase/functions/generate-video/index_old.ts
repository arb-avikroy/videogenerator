...
serve(async (req) => {
  ...
  try {
    ...
    const { scenes, guestSessionId, scriptTitle, generationId } = body;
    ...
    // Save to database - update videos array
    if (generationId && (userId || guestSessionId) && results.length > 0) {
      try {
        // First fetch the current videos array
        const { data: currentGen } = await supabaseClient
          .from('generations')
          .select('videos, narration_audio')
          .eq('id', generationId)
          .single();

        const currentVideos = currentGen?.videos || [];
        const currentAudio = currentGen?.narration_audio || [];

        // Add new videos and audio
        const newVideos = results.map(r => ({
          sceneNumber: r.sceneNumber,
          videoUrl: r.video,
          error: r.error
        }));

        const newAudio = results
          .filter(r => r.audio)
          .map(r => ({
            sceneNumber: r.sceneNumber,
            audioUrl: r.audio
        ...
        const { error: dbError } = await supabaseClient
          ...
          .update({
            ...
            narration_audio: [...currentAudio, ...newAudio]
          })
          .eq('id', generationId);

        if (dbError) {
          console.error('Error updating videos in database:', dbError);
        } else {
          console.log('Videos added to generation:', generationId);
        }
      } catch (dbError) {
